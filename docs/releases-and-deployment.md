...
5. Release publication ends here. Deployment independently consumes completed releases; neither GitHub nor GitLab CI connects to the application host.

Merging to `main` enables release publication after checks pass. A completed release can be picked up by the independently configured deployment controller. Deployment setup, pausing updates, and operational logs belong in the private infrastructure repository.
...
- Failed checks publish nothing. Failed image jobs may leave partial image tags or untagged candidates, but create no GitHub Release.
...
- Rerunning an already-published release does not allocate another version or change the latest release. Finishing an older release does not mark it latest. Deployment and application rollback remain separate from publication, keeping migrations forward-only.
...
Pause automatic deployment using the private runbook if the recovered release should not deploy yet. No credential changes are required for the collision fix; the reservation job uses the workflow token's existing repository-write permission, moved earlier in the pipeline.

## Operator setup

No GitLab trigger or deployment credential is required. If previously configured, the operator should revoke the old dedicated GitLab trigger token and remove the GitHub Actions secrets `GITLAB_TRIGGER_URL` and `GITLAB_TRIGGER_TOKEN` and repository variable `ENABLE_GITLAB_HANDOFF`. Do not remove unrelated infrastructure credentials. SSH/VPN credentials and host settings stay outside this repository and its Actions jobs; cleanup does not require an agent to inspect credential values.

Quality jobs have read-only repository permissions. Image and promotion jobs add package writes. Version reservation and release creation have repository writes and read-only package access. The reusable-workflow caller grants the maximum needed permissions, which its individual jobs reduce. Checkout does not persist credentials. GitHub API requests reject redirects rather than forwarding their scoped token.
...
Private GitLab CI validates deployment code and configuration only. There is no deployment trigger job or CI-to-host access to configure.
...
Validate release allocation, immutable-tag guards, partial failures, stale retries, and GitHub API redirect protection without network access or real credentials:

```bash
python3 -B -m unittest discover -s scripts -p 'test_*.py'
```

These tests use temporary Git repositories and mocked registry/GitHub operations. They do not prove live registry permissions or end-to-end CI execution. Confirm all four tags/digests and notes in the next observed release.

Reference: [GitHub reusable workflows](https://docs.github.com/en/actions/how-tos/reuse-automations/reuse-workflows).
